...
public class Controller implements Initializable {
    ...
    public void atualizar() {
        botaoBatata.setText("Batata x " + fazenda.getCeleiro().getQtde(Produtos.batata));
        botaoCenoura.setText("Cenoura x " + fazenda.getCeleiro().getQtde(Produtos.cenoura));
        botaoMorango.setText("Morango x " + fazenda.getCeleiro().getQtde(Produtos.morango));
            ...
            for (int y = 0; y < 13; y++) {
                ...
                if (terreno.getPlanta() != null) {
                    imageView.setImage(new Image(
                        getClass().getResourceAsStream(terreno.getPlanta().getImagem())
                    ));
                } else {
        ...
        grid.setOnMouseClicked(e -> {
            ...
            try {
                if (botaoBatata.isSelected()) {
                    fazenda.plantar(new Batata(), x, y);
                } else if (botaoCenoura.isSelected()) {
                    fazenda.plantar(new Cenoura(), x, y);
                } else if (botaoMorango.isSelected()) {
                    fazenda.plantar(new Morango(), x, y);
